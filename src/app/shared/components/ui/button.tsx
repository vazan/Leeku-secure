...
const buttonVariants = cva(
  "group/button inline-flex shrink-0 cursor-pointer items-center justify-center rounded-[var(--radius-md)] border border-transparent bg-clip-padding text-sm font-medium whitespace-nowrap text-[var(--text-primary)] transition-[background-color,border-color,color,box-shadow,transform] duration-[var(--t-control)] ease-[var(--ease-control)] outline-none select-none focus-visible:border-[var(--accent-linear)] focus-visible:ring-3 focus-visible:ring-[color-mix(in_srgb,var(--accent-linear)_35%,transparent)] active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
        default:
          "bg-[var(--accent-linear)] text-[var(--text-primary)] shadow-[var(--shadow-hairline)] hover:bg-[var(--accent-linear-bright)]",
        outline:
