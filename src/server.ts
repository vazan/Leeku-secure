  ...
  try {
    ...
    const fileQuery = `
      SELECT id, owner_user_id, original_name_encrypted, original_name_iv, original_name_auth_tag,
             ...
             scan_result, scan_message, is_encrypted, leeku_vibe, ttl_hours, expires_at, created_at
      FROM files 
      WHERE id IN (${fileIds.map((_, i) => `@fileId${i}`).join(', ')})
    `;
...
app.post('/api/sharing/folder-links/:id/remove', authenticateUser as express.RequestHandler, async (req: AuthenticatedRequest, res) => {
  try {
    const request = await getRequest();
    const linkId = getSingleParam(req.params.id);
    request.input('id', sql.UniqueIdentifier, linkId);
    request.input('ownerId', sql.UniqueIdentifier, req.userId!);
    ...
    await logSystemEvent(req.userId!, req.user!.username, 'Delete', 'FolderShareLink', linkId, req, `Removed share link for folder ${result.recordset[0].folder_id}.`);
    res.json({ success: true });
  } catch (err) {
    ...
    if (!existing.recordset.length) {
      ...
      const insRes = await insReq.query<ShareRow>(
         ...
         OUTPUT INSERTED.id,INSERTED.file_id,INSERTED.public_token,INSERTED.password_hash,
            INSERTED.expires_at,INSERTED.max_downloads,INSERTED.download_count,INSERTED.is_active,INSERTED.allow_external_preview,INSERTED.allow_decrypted_external_preview,INSERTED.created_at
...
async function ensureOptionalShareLinkColumns(): Promise<void> {
  const request = await getRequest();
  await request.query(`
    IF OBJECT_ID('dbo.share_links', 'U') IS NOT NULL
    BEGIN
      IF COL_LENGTH('dbo.share_links', 'allow_external_preview') IS NULL
        ALTER TABLE dbo.share_links ADD allow_external_preview BIT NOT NULL CONSTRAINT DF_share_links_allow_external_preview DEFAULT(0);
      IF COL_LENGTH('dbo.share_links', 'allow_decrypted_external_preview') IS NULL
        ALTER TABLE dbo.share_links ADD allow_decrypted_external_preview BIT NOT NULL CONSTRAINT DF_share_links_allow_decrypted_external_preview DEFAULT(0);
    END
  `);
}
...
async function ensureFileFolderSchema(): Promise<void> {
  ...
  await request.query(`
    ...
    END;
    IF COL_LENGTH('file_folders', 'parent_folder_id') IS NULL
      ALTER TABLE file_folders ADD parent_folder_id UNIQUEIDENTIFIER NULL;
    IF NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name='FK_file_folders_parent')
      ALTER TABLE file_folders ADD CONSTRAINT FK_file_folders_parent FOREIGN KEY (parent_folder_id) REFERENCES dbo.file_folders(id) ON DELETE NO ACTION;

    IF EXISTS (SELECT 1 FROM sys.objects WHERE type='UQ' AND name='UQ_file_folders_owner_name')
      ALTER TABLE file_folders DROP CONSTRAINT UQ_file_folders_owner_name;
    IF NOT EXISTS (SELECT 1 FROM sys.objects WHERE type='UQ' AND name='UQ_file_folders_owner_parent_name')
      ALTER TABLE file_folders ADD CONSTRAINT UQ_file_folders_owner_parent_name UNIQUE (owner_user_id, parent_folder_id, name);

    IF EXISTS (SELECT 1 FROM sys.indexes WHERE name='IX_file_folders_owner' AND object_id=OBJECT_ID('dbo.file_folders'))
      DROP INDEX IX_file_folders_owner ON dbo.file_folders;
    IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name='IX_file_folders_owner_parent' AND object_id=OBJECT_ID('dbo.file_folders'))
      CREATE INDEX IX_file_folders_owner_parent ON dbo.file_folders(owner_user_id, parent_folder_id, name);
