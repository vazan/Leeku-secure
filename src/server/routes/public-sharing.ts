      ...
      const filesResult = await filesRequest.query<{
        ...
        encrypted_key: Buffer; key_iv: Buffer; key_auth_tag: Buffer; file_iv: Buffer; file_auth_tag: Buffer;
         ...
         `WITH RECURSIVE folder_tree AS (
            SELECT id,parent_folder_id,CAST('' AS text) AS relative_path,0 AS depth FROM file_folders WHERE id=@folderId
           UNION ALL
            SELECT ff.id,ff.parent_folder_id,
              CAST(CASE WHEN ft.relative_path='' THEN ff.name ELSE ft.relative_path||'/'||ff.name END AS text),
              ft.depth+1
            FROM file_folders ff INNER JOIN folder_tree ft ON ff.parent_folder_id=ft.id
            WHERE ft.depth<5
         )
          SELECT f.id AS file_id,
            CASE WHEN ft.relative_path='' THEN '' ELSE ft.relative_path||'/' END AS relative_path,
